...
// processRequest(request, response);
      ...
      String uploaddate=db.getDate();
       String str=new java.text.SimpleDateFormat("ddMMyyyyhhmmss").format(new java.util.Date());
        ...
        if(db.nonQuery(q)==true)
        {
               File f=new File(request.getRealPath("/uploadtender"),filename);  
                 Files.copy(is,f.toPath());
                         response.getWriter().print("<script>alert('tender is uploaded');window.location.href='adminzone/uploadtender.jsp';</script>");
        ...
        {
        response.getWriter().print("<script>alert('tender is not upload');window.location.href='adminzone/uploadtender.jsp';</script>");
        }
        }
        
            catch(Exception ex)
                    {
                    response.getWriter().print("<script>alert('error1');</script>");
